...
root_order = 2
corr_window=15
extra_label='_3_extend'
file='input_JP_7.7.csv'
...
#stations = './data/wilber/wilber-stations.txt' #'./data/'+str(res['Event'])+'_bp_stations_'+str(res['Array_name'])+'/*'
#waveforms= './data/pyweed/Mw_7.7/*.mseed'   # wilber/2023-02-06-mww78-turkey.miniseed #'./data/'+str(res['Event'])+'_bp_waveforms_'+str(res['Array_name'])+'/*.mseed'
stations = str(res['stations']) #'./data/wilber/wilber-stations.txt' #'./data/'+str(res['Event'])+'_bp_stations_'+str(res['Array_name'])+'/*'
waveforms= str(res['waveforms']) 

#'./data/pyweed/Mw_7.7/*.mseed'   # wilber/2023-02-06-mww78-turkey.miniseed #'./data/'+str(res['Event'])+'_bp_waveforms_'+str(res['Array_name'])+'/*.mseed'
...
#Exp_name=str(Array_name)+'_'+str(event_depth)+'km_'+str(bp_l)+'_'+str(bp_u)+'Hz_'+str(res['model'])+'_'+str(source_grid_size)+'_grid'+str(extra_label)
Exp_name=str(Array_name)+'_'+str(event_depth)+'km_'+str(res['model'])+'_'+str(source_grid_size)+'_grid'+str(extra_label)

print('Working in Exp:',Exp_name)
...
for t in stream_work:
        ...
        if sta in sta_name:
            ...
            if sta_azimuth[ind] > 0:
                t.stats['Backazimuth']       = sta_azimuth[ind]
            else:
                 t.stats['Backazimuth']       = 360-sta_azimuth[ind]
            #t.stats['backazimuth']       = sta_backazimuth[ind]
...
stream_SPS = bp_lib.check_sps(stream_work)
...
#stream_SPS_dist_azimuth = bp_lib.check_azimuth_except(stream_SPS_dist_azimuth,azimuth_min,azimuth_max)
...
stream_SPS_dist_azimuth_SNR = bp_lib.snr_check(stream_SPS_dist_azimuth_SNR,SNR,corr_window,corr_window)
...
##########################################################################
#Ref_station_index=bp_lib.get_ref_station(stream_SPS_dist_azimuth_SNR,'mean','')
...
#stream_correlated=bp_lib.crosscorr_stream(stream_correlated,ref_trace,5)
stream_correlated=bp_lib.crosscorr_stream_xcorr(stream_correlated,ref_trace,corr_window)
...
print('Total no of traces after data gap checks:', len(stream_cut))


stream_cut_correlated=stream_cut.copy()
#Ref_station_index=bp_lib.get_ref_station(stream_cut_correlated,'mean','')
Ref_station_index=bp_lib.get_ref_station(stream_cut_correlated)
ref_trace = stream_cut_correlated[Ref_station_index]
print(Ref_station_index)
print(ref_trace)
#stream_cut_filtered_correlated=bp_lib.crosscorr_stream(stream_cut_filtered,ref_trace,5)
#stream_correlated = stream_SPS_dist_azimuth_SNR.copy()
...
stream_for_bp=stream_cut_correlated.copy()
...
print("Progress back-projection DONE for Exp:",  Exp_name)
